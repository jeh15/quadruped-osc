...
def loss_function(
    ...
    value_network_type: Literal['feedforward', 'transformer'] = 'feedforward',
    ...
    # Split key:
    policy_dropout_key, value_dropout_key, distribution_key = jax.random.split(
        rng_key, 3,
    )
    ...
    logits = policy_apply(
        ...
        train=True,
        dropout_key=policy_dropout_key,
    )
    if value_network_type == 'feedforward':
        values = value_apply(
        ...
        values = value_apply(
            ...
            inputs=data.observation,
            train=True,
            dropout_key=value_dropout_key,
        )
        bootstrap_values = value_apply(
            normalization_params,
            params.value_params,
            inputs=data.next_observation[-1],
            train=True,
            dropout_key=value_dropout_key,
        )

...
def train(
    ...
    osc_controller = controller.OSCController(
        ...
        num_contacts=4,
        num_taskspace_targets=num_taskspace_targets,
    ...
    get_data_fn = jax.jit(
        jax.vmap(osc_utils.get_data, in_axes=(None, 0, 0, None))
    )
    update_fn = jax.jit(
        jax.vmap(osc_controller.update, in_axes=(0, 0))
    )
    warmstart_fn = functools.partial(
        osc_controller.warmstart, batch_size=num_envs,
    )
    warmstart_fn = jax.jit(warmstart_fn)
    solve_fn = functools.partial(osc_controller.solve, batch_size=num_envs)
    solve_fn = jax.jit(solve_fn)

    # Initialize QP Warmstart:
    points = env.taskspace_points(env_state.pipeline_state)

    osc_data = get_data_fn(
        env.sys.mj_model,
        env_state.pipeline_state,
        points,
        osc_controller.body_ids,
    )
    prog_data = update_fn(jnp.zeros((num_taskspace_targets, 6)), osc_data)
    warmstart = warmstart_fn(prog_data)
    ...
    def training_step(
        ...
        # Generates Episode Data: (Add OSC into the loop here...)
